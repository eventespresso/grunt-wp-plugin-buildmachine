...
module.exports = {
    ...
    setPluginParams: function( task ) {
        var pluginSlug = grunt.config.get( 'currentSlug' ),
            ...
            params = grunt.file.exists( src ) ?
                grunt.file.readJSON( src ) :
                null,
            gitinfo = grunt.config.get( 'gitinfo' );

        if ( params === null ) {
            grunt.fail.warn(
                'The repo must have a valid info.json file in it with params ' +
                'for the remaining tasks.  Maybe you had a typo in the ' +
                'provided plugin slug?'
            );
        }
        ...
        if ( typeof gitinfo.local === 'undefined' ) {
            grunt.fail.warn(
                'git info did not appear to work. It is needed to be able ' +
                'to complete tasks. So aborting.'
            );
        }

        // let's setup certain environment variables
        // based on the git info we've received.
        if ( gitinfo.local.branch.current.name === 'master' ) {
            params.versionType = 'rc';
            params.branch = 'master';
        } else {
            params.versionType = params.branch =
        ...
        if ( gitinfo.local.branch.current.lastCommitMessage ) {
            grunt.config.set(
                ...
                gitinfo.local.branch.current.lastCommitMessage
            );
        }
        ...
        grunt.config.set(
            'shell.remove_folders_release.command',
            this.utils.rmPrepareFolders( params.releaseFilesRemove ).join( ';' )
        );
        grunt.config.set(
            'shell.remove_folders_decaf.command',
            this.utils.rmPrepareFolders( params.decafFilesRemove ).join( ';' )
        );
    },
    initPluginSlug: function( pluginSlug, validate ) {
        ...
        if ( validate && ! grunt.file.exists( src ) ) {
            grunt.fail.warn(
                'There is no builder repo setup for ' + pluginSlug + '.  ' +
                'Please doublecheck your spelling, or you may need to run ' +
                'builder:init to get that repo setup first.'
            );
        }
    ...
    initializeFromMap: function() {
        ...
        grunt.registerTask(
            ...
            'Write a map of all initialized things to the generated map ' +
            'of origin_repo_address: directoryname',
            function() {
                grunt.file.write(
        ...
        grunt.log.oklns(
            'Finished initializing all remotes in the buildmap.json file' );
    },
    initializeSrc: function( remotes, generatedMap ) {
        if ( typeof remotes.origin === 'undefined' ) {
            grunt.fail.warn(
                'Cannot build because there is no origin defined for the remotes.'
            );
            return;
        }
        var encryptedSlug = crypto.createHash( 'md5' )
                .update( remotes.origin )
                .digest( "hex" ),
            initObject = this,
            tasksToRun = [
        ...
        grunt.registerTask(
            ...
            function() {
                initObject.queueNewShellTasksForBuilder(
                    remotes.origin,
                    remotes,
                    generatedMap
                );
    ...
    queueNewShellTasksForBuilder: function(
        originRepoAddress,
        remotesToRegister,
        generatedMap
    ) {
        ...
        for ( var remoteName in remotesToRegister ) {
            if ( remoteName !==
                'origin' &&
                remotesToRegister.hasOwnProperty( remoteName ) ) {
                remoteRegistrationCommand.push(
                    'git remote remove ' +
                    remoteName +
                    '; yes | git remote add -f ' +
                    remoteName +
                    ' ' +
                    remotesToRegister[ remoteName ]
                );
            }
        }
    ...
    queueSettingPluginSlugForBuild: function( originRepoAddress ) {
        ...
        grunt.registerTask(
            ...
            'Sets the plugin slug from the temp directory',
            function() {
                // k now the repo should be cloned,
                // so we should be able to grab info from the info.json
                info = grunt.file.exists( infoPath ) ?
                    grunt.file.readJSON( infoPath ) :
                    null;
                if ( info === null || typeof info.slug === 'undefined' ) {
                    grunt.fail.warn(
                        'Unable to get the info.json from the cloned ' +
                        'repository.  Make sure all origin remote addresses ' +
                        ' have an info.json file in the root directory.'
                    );
                }
                grunt.config.set( 'currentSlug', info.slug );
                grunt.log.oklns(
                    'Finished grabbing the plugin slug from the origin repo.'
                );
            }
        );
        grunt.config.set( 'shell', shellConfig );
