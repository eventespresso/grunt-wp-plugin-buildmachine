...
$info_json = $info_json_file ? json_decode(file_get_contents($info_json_file), false) : '';
...
// and make sure we have at least 3 periods if not we append a period
if ($type === 'rc' && substr_count($version_to_split, '.') < 3) {
...
switch ($type) {
    case 'pre_release':
        // we're not bumping just replacing whatever string is in the version string with 'beta', since pre-releases
        // are ONLY built on top of rc/alpha/beta builds, then we just replace the second from last array index
    ...
    case 'decaf':
        // we're not bumping, just replacing whatever string is in the version string with 'decaf',
        // and dropping the last version numbers.
        // Since decaf is built on top of checked out tags, we'll just replace the second from last array index.
    ...
    case 'minor':
        if (count($version_split) === 5) {
            // we don't bump any versions here because it should
            // already have been bumped in prepping rc version.
            $version_split[3] = $version_strings['release'];
            unset($version_split[4]);
        } else {
            $index = count($version_split) - 2;
...
if ($do_info_json && $info_json_file && ! empty($new_version)) {
    $info_json = json_decode(file_get_contents($info_json_file), false);
